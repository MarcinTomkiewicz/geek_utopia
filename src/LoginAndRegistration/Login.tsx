...
export const Login = ({ isModal }: any) => {
	...
	return (
		<Form
			...
			className={isModal ? "user__form--modal" : "user__form"}
		>
			<h4 className="my-3">
				{isModal ? "" : language.labels?.logging[langCode]}
			</h4>
			{currentUser === undefined || language.labels === undefined ? (
				<BusyBox size={60} />
			) : (
				<TextInput
					input={`${language.labels?.email[langCode]}`}
					isRequired="true"
					type="email"
					name="email"
					data={user}
			...
			{currentUser === undefined || language.labels === undefined ? (
				...
				<TextInput
					input={`${language.labels?.password[langCode]}`}
					isRequired="true"
					type="password"
					name="password"
					data={user}
				...
				<OverlayTrigger
					...
					overlay={
						<Tooltip id="tooltip-login">
							{language.tooltips?.login_fill[langCode]}
						</Tooltip>
					}
				>
					<span className="d-inline-block">
						{currentUser === undefined ||
						language.labels === undefined ? (
							<BusyBox size={60} />
						) : (

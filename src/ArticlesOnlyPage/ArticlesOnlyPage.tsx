...
export const ArticlesOnlyPage = ({ articleType, defaultPostsOnPage }: ArticlesPageProps): any => {
  ...
  const badgeBackground = ["primary", "secondary", "success", "danger", "warning", "info", "dark"];

  return (
    <>
      <h1 className="py-3">Newsy</h1>
      <div className="d-flex flex-column gap-2 w-100">
        <div className="d-flex justify-content-between align-items-center flex-column w-100 mb-3 p-2 article__list no--opacity" key={news[0]?.id}>
          <div className="d-flex justify-content-center w-100">
            <Link to={`/news/${news[0]?.id}`} className="general__text" style={{ cursor: "pointer" }}>
              <img src={news[0]?.picture} alt={news[0]?.title} style={{ borderRadius: "10px", width: "100%" }} />
            </Link>
          </div>
          <div className="d-flex mt-2 justify-content-start align-items-start flex-column w-100 h-100">
            <Link to={`/news/${news[0]?.id}`} className="general__text" style={{ cursor: "pointer" }}>
              <div className="d-flex align-items-center flex-row w-100">
                <h2 style={{ marginBottom: "0" }}>{news[0]?.title} </h2>
                {news[0]?.is_adult ? (
                  <h5
              ...
              <div className="d-flex justify-content-between align-items-center flex-row w-100">
                <div style={{ fontSize: "0.75rem" }}>{generateDate(news[0])}</div>
              </div>
            </Link>
            <div className="d-flex flex-row justify-content-start gap-2 align-items-center mt-1 mb-3 flex-wrap w-100">
              {news[0]?.tags[0] !== "" ? news[0]?.tags.map((tag: any) => {
                return <Badge bg={badgeBackground[Math.floor(Math.random() * badgeBackground.length)]}>{tag}</Badge>;
              }) : "Nie ma żadnych tagów do tego artykułu"}
            </div>
            <div>
              {news[0]?.short_descr === undefined || news[0]?.short_descr === "" ? news[0]?.content.substring(0, 500) : news[0]?.short_descr}
              ...
            </div>
          </div>
        </div>
      ...
      <div className="d-flex flex-row justify-content-between align-items-center w-100 mb-3 w-75">
        ...
        <div className="d-flex flex-row justify-content-around align-items-center flex-1 gap-2">
          ...
          <button onClick={handlePostsOnPageChange} style={{ background: "#a10eb8", color: "#f7fcfc", border: "white 1px solid", borderRadius: "5px" }} className="text-center" value={1}>
            10
          </button>
          <button onClick={handlePostsOnPageChange} style={{ background: "#a10eb8", color: "#f7fcfc", border: "white 1px solid", borderRadius: "5px" }} className="text-center" value={2}>
            20
          </button>
          <button onClick={handlePostsOnPageChange} style={{ background: "#a10eb8", color: "#f7fcfc", border: "white 1px solid", borderRadius: "5px" }} className="text-center" value={5}>
            50
          </button>
        </div>
      </div>
            ...
            <div key={article?.id} className="article__wrap--children">
              <div className="d-flex justify-content-between align-items-start flex-column w-100 mb-3 p-2 article__list" style={{height: "calc(100% - 1rem)"}}key={article?.id}>
                ...
                <div className="d-flex mt-2 mx-3 justify-content-start align-items-start flex-column w-100 h-100">
                  ...
                  <div className="d-flex flex-row justify-content-start gap-2 align-items-center mt-1 mb-3 flex-wrap w-100">
                    {article?.tags[0] !== "" ? article?.tags.map((tag) => {
						if (tag === "") {
							return;
						}
                      return <Badge bg={badgeBackground[Math.floor(Math.random() * badgeBackground.length)]}>{tag}</Badge>;
